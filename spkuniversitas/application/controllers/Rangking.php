...
class Rangking extends MY_Controller
{
    ...
    public function index()
    {
         ...
         */
        $table1 = $this->initialTableSAW($saham);
    ...
    private function initialTableSAW($universitas)
    {
        ...
        $dataInput = array();
        $no = 0;
        foreach ($saham as $item => $itemSaham) {
            foreach ($nilai as $index => $itemNilai) {
                if ($itemSaham->kdSaham == $itemNilai->kdSaham) {
                    $dataInput[$no]['saham'] = $itemSaham->saham;
